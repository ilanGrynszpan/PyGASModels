...
class ZAGB2LogLinkDistribution2(Distribution):
    def __init__(self):
    ...
    def logpdf(self, y, **kwargs):
        ...
        if y <= 0:
            ...
            return (
                ...
                - phi
                - np.log(beta(xi, zeta))
                - (xi + zeta) * np.log(1 + (y / np.exp(phi)) ** (1 / gamma))
            )
    ...
    def score(self, y, **kwargs):
        ...
        if y <= 0:
            ...
            dL_dphi = (
                ...
                - (xi / zeta)
                + ((xi + zeta) * ((y / np.exp(phi)) ** (1 / gamma)) * np.exp(phi))
                / (gamma * (1 + (y / np.exp(phi)) ** (1 / gamma)))
            )
    ...
    def cdf(self, y, **kwargs):
        ...
        # π = logistic(delta_0 + delta_1 * phi)
        linear_comb = delta_0 + delta_1 * phi
        pi = expit(linear_comb)
        ...
        if y < 0:
            ...
            # GB2 parameters according to your specification:
            b = np.exp(phi)  # scale parameter
            a = 1 / gamma  # shape parameter 1
            p = xi  # shape parameter 2
            q = zeta  # shape parameter 3

            # GB2 CDF: F(y) = I_{z/(1+z)}(p, q) where z = (y/b)^a
            z = (y / b) ** a
            gb2_cdf = betainc(p, q, z / (1 + z))
    ...
    def mean(self, **kwargs):
        ...
        # GB2 component mean
        gb2_mean = np.exp(phi) * beta(xi + gamma, zeta - gamma) / beta(xi, zeta)
    ...
    def variance(self, **kwargs):
        ...
        ex = np.exp(phi) * beta(xi + gamma, zeta - gamma) / beta(xi, zeta)

        ex2 = np.exp(phi) ** 2 * beta() / beta(np.exp(xi), np.exp(zeta))
